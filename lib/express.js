...
module.exports = function initialize(adapter, opts) {
  ...
  router.get('/b/:id/height', (req, res) => {
    ...
    rpc('getblockheader', [req.params.id, true], (err, json) => respond(req, res, err, json && json.height ? json.height : 0))
  })
  ...
  router.get('/a/:address/txs', (req, res) => {
    ...
    let vin = req.query.vin && req.query.vin == '1' ? true : false
    ...
    adapter.transactionIdsByScriptId(scId, height, (err, txIdSet) => {
      ...
      if (vin) {
        parallel(task2s, (err, txSet) => {
          if (err) return respond(req, res, err)

          let task3s = {}
          for (let txId in txSet) {
            let tx = txSet[txId]
            for (let v in tx.vin) {
              let vtxid = tx.vin[v].txid;
              if (!task3s[vtxid]) {
                task3s[vtxid] = (next) => rpc('getrawtransaction', [vtxid, verbose], next)
              }
            }
          }

          parallel(task3s, (err, vtxSet) => {
            if (err) return respond(req, res, err)

            for (let txId in txSet) {
              let tx = txSet[txId]
              for (let vi in tx.vin) {
                let v = tx.vin[vi]
                v.voutValue = vtxSet[v.txid].vout.filter(vo => vo.n == v.vout)[0];
              }
            }
            ...
            for (let i in txSet) {
              let tx = txSet[i]
              if (tx.blockhash && !task4s[tx.blockhash]) {
                task4s[tx.blockhash] = (next) => adapter.blockExtraInfoByTransactionId(tx.txid, (serr, sresult) => {
                  next(serr, { txid: tx.txid, extra: sresult })
                })
              }
            }

            parallel(task4s, (err, heightSet) => {
              if (err) return respond(req, res, err)

              for (let i in heightSet) {
                let hs = heightSet[i];
                if (hs) {
                  if (hs.extra.height) {
                    txSet[hs.txid].height = hs.extra.height;
                  }
                  if (!txSet[hs.txid].time && hs.extra.receivedDate) {
                    txSet[hs.txid].time = hs.extra.receivedDate;
                  }
                }
              }

              respond(req, res, err, txSet)
            });
          })
        })
      } else {
        parallel(task2s, (err, txSet) => respond(req, res, err, txSet))
      }
  ...
  router.post('/x/:xpubKey/checkaddrs', bodyParser.json(), (req, res) => {
    ...
    let xpubKeyHex
    if (req.params.xpubKey.length == 64) {
      xpubKeyHex = req.params.xpubKey
    } else {
      try {
    ...
    adapter.seenScriptId(scId, (err, result) => {
      ...
      adapter.addToXpubKey(scId, xpubKeyHex, (serr) => {
        ...
        if (result) {
          ...
          callback(null, {
            index: start,
            address: isBCH() ? bcoinsdk.bcoin.toCashAddressForBCH(address).data : address
          })
        }
      })
    });
  ...
  router.get('/x/:xpubKey/txs', (req, res) => {
    ...
    adapter.scriptIdsByXpubKey(xpubKey, (err, scIds) => {
      ...
      parallel(task1s, (err, txIdSet) => {
        ...
        let task2s = {}
        for (let i in txIdSet) {
          for (let txId in txIdSet[i]) {
            task2s[txId] = (next) => rpc('getrawtransaction', [txId, verbose], next)
          }
        }
  ...
  router.get('/t/:id', (req, res) => {
    ...
    let verbose = req.query.verbose && req.query.verbose == '1' ? true : false
    let vin = req.query.vin && req.query.vin == '1' ? true : false
    rpc('getrawtransaction', [req.params.id, verbose], (err, tx) => {
      if (err) return respond(req, res, err)
      ...
      if (vin) {
        ...
        for (let v in tx.vin) {
          let vtxid = tx.vin[v].txid;
          if (!tasks[vtxid]) {
            tasks[vtxid] = (next) => rpc('getrawtransaction', [vtxid, verbose], next)
          }
        }
        parallel(tasks, (err, vtxSet) => {
          if (err) return respond(req, res, err)
          ...
          //if (tx.blockhash) {
            adapter.blockExtraInfoByTransactionId(tx.txid, (err, extra) => {
              if (extra) {
                if (!extra.height) {
                  tx.height = extra.height;
                }
                if (!tx.time && extra.receivedDate) {
                  tx.time = extra.receivedDate;
                }
              }
              respond(req, res, err, tx)
            });
          //} else {
            //respond(req, res, err, tx)
          //}
