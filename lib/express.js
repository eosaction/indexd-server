...
module.exports = function initialize(adapter, opts) {
  ...
  let network = bitcoin.networks[currency]
  ...
  */
  ...
  router.get('/i/:xpubKey', (req, res) => {
    ...
    checktx(params, (err, result) => respond(req, res, err, result));
  })

  function checktx(params, callback) {
    ...
    adapter.seenScriptId(scId, (err, result) => {
      ...
      if (result) {
        checktx(xpubKey, change, start + 1, callback)
      } else {
  ...
  router.get('/l/:label/addaddr', (req, res) => {
    ...
    var addrdata = {
      xpubKey: req.query.xpubKey,
      change: req.query.change,
      index: req.query.index,
      address: req.query.address
    };
    var isvalid = hdwsdk.hdWallet.validateAddressByXpubKey(addrdata);
    if (!isvalid.status || !isvalid.data)
      return respond(req, res, 400)
