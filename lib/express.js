          ...
          parallel(task3s, (err, vtxSet) => {
            ...
            for (let i in txSet) {
              ...
              if (tx.blockhash && !task4s[tx.blockhash]) {
                task4s[tx.blockhash] = (next) => adapter.blockchain.blockHeightByTransactionId(tx.txid, (serr, sresult) => {
                  next(serr, { txid: tx.txid, height: sresult })
                })
              }
            }
            ...
            parallel(task4s, (err, heightSet) => {
              ...
              for (let i in heightSet) {
                let hs = heightSet[i];
                if (hs && hs.height) {
                  txSet[hs.txid].height = hs.height;
                }
              }
    ...
    rpc('getrawtransaction', [req.params.id, verbose], (err, tx) => {
      ...
      if (vin) {
        ...
        parallel(tasks, (err, vtxSet) => {
          ...
          if (tx.blockhash) {
            adapter.blockchain.blockHeightByTransactionId(tx.txid, (err, height) => {
              if (err) return respond(req, res, err)
              if (height) {
                tx.height = height;
              }
