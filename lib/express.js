          ...
          parallel(task3s, (err, vtxSet) => {
            ...
            for (let txId in txSet) {
              ...
              if (tx.blockhash && !task4s[tx.blockhash]) {
                task4s[tx.blockhash] = (next) => rpc('getblockheader', [tx.blockhash, true], next)
              }
            }

            parallel(task4s, (err, blockSet) => {
              if (err) return respond(req, res, err)

              for (let i in blockSet) {
                let block = blockSet[i];
                if (block && block.height) {
                  Object.keys(txSet).filter(tx => txSet[tx].blockhash && txSet[tx].blockhash == block.hash).forEach(tx => {
                    txSet[tx].height = block.height;
                  });
                }
              }
    ...
    rpc('getrawtransaction', [req.params.id, verbose], (err, tx) => {
      ...
      if (vin) {
        ...
        parallel(tasks, (err, vtxSet) => {
          ...
          if (tx.blockhash) {
            rpc('getblockheader', [tx.blockhash, true], (err, block) => {
              if (err) return respond(req, res, err)
              if (block && block.height) {
